...
public class Board {
  ...
  public void clearFullRows() {
    ...
    reverseRangeY()
            .filter(y -> isRowFull(y))
            .forEach(this::clearRow);
  }

  /**
   * For every valid y coordinate (0-20), calls printLine on that y to output it as a properly formatted String.
   * Then joins each of these line Strings, using line breaks as delimeters and suffixes.
   * So each line is added to the returned String on an actual new line.
   * @return The new String made from joining all line Strings of valid y coordinates.
   */
  ...
  /**
   * @param y the Y coordinate of the line in question
   * Starts with a stream of valid x coordinates (0-10)
   * Maps each x to a String through the s value of the Color representing it
   * Now that we have a Stream of Strings representing the board, we join these into a single String
   * And split each entry with "|" for readability.
   * @return The String created by joining the s values of every Color in a row at a given y coordinate.
   */
